...
class FpgnnModel(nn.Module):
    ...
    def forward(self, smiles):
        fp_list = calculate_RDK_fingerprints(smiles)
...
for file_name in csv_files:
    ...
    best_epoch = 0
    best_model_path = f"29-GAT-RDK-{file_name.split('.')[0]}-model.pth"
    ...
    results_name = f"29-GAT-RDK-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'29-GAT-RDK Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"29-GAT-RDK-{file_name.split('.')[0]}.png"
