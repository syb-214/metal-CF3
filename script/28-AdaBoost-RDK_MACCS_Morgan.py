...
def calculate_RDK_fingerprints(smiles, radius=2, nBits=2048):
    fingerprints = []
    for s in smiles:
...
for file_name in csv_files:
    ...
    RDK_fingerprints = calculate_RDK_fingerprints(smiles)
    MACCS_fingerprints = calculate_MACCS_fingerprints(smiles)
    Morgan_fingerprints = calculate_Morgan_fingerprints(smiles)

    # Combine fingerprints
    combined_fingerprints = [rdk + maccs + morgan for rdk, maccs, morgan in zip(RDK_fingerprints, MACCS_fingerprints, Morgan_fingerprints)]

    # Convert combined fingerprints to DataFrame
    df_combined = pd.DataFrame(combined_fingerprints, index=smiles.index)
    ...
    model_filename = "28-AdaBoost-RDK_MACCS_Morgan-" + file_name.split('.')[0] + "-model.joblib"
    dump(best_model, model_filename)

    # Save the best parameters
    best_params_name = "28-AdaBoost-RDK_MACCS_Morgan-" + file_name.split('.')[0] + "-best_params.txt"
    ...
    results_name = "28-AdaBoost-RDK_MACCS_Morgan-" + file_name.split('.')[0] + "-results.txt"
    results = {
    ...
    cv_scores_name = "28-AdaBoost-RDK_MACCS_Morgan-" + file_name.split('.')[0] + "-cv_scores.txt"
    ...
    plt.title('28-AdaBoost-RDK_MACCS_Morgan Prediction', fontsize=16)
    ...
    # Save the image
    image_name = "28-AdaBoost-RDK_MACCS_Morgan-" + file_name.split('.')[0] + ".png"
