...
class FpgnnModel(nn.Module):
    def __init__(self, fp_dim_rdk, fp_dim_maccs, fp_dim_morgan, hidden_dim, dropout, nfeat, nhid, nheads):
        super(FpgnnModel, self).__init__()
        self.fpn_rdk = FPN(fp_dim_rdk, hidden_dim, dropout)
        ...
        self.fc = nn.Linear(hidden_dim * 3 + nfeat, 1)  # Adjusted for combined fingerprints
    ...
    def forward(self, smiles):
        ...
        combined_out = torch.cat([fpn_out_rdk, fpn_out_maccs, fpn_out_morgan, gat_out], dim=1)
...
for file_name in csv_files:
    ...
    model = FpgnnModel(
        fp_dim_rdk=2048, fp_dim_maccs=167, fp_dim_morgan=2048, 
    ...
    best_epoch = 0
    best_model_path = f"35-GAT-RDK_MACCS_Morgan-{file_name.split('.')[0]}-model.pth"
    ...
    results_name = f"35-GAT-RDK_MACCS_Morgan-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'32-GAT-RDK_MACCS_Morgan Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"35-GAT-RDK_MACCS_Morgan-{file_name.split('.')[0]}.png"
