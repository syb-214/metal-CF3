...
# Define the FpgnnModel
class FpgnnModel(nn.Module):
    def __init__(self, fp_dim, hidden_dim, dropout, nfeat, nhid, nheads):
        super(FpgnnModel, self).__init__()
        self.fpn = FPN(fp_dim, hidden_dim, dropout)
        self.gat = GATEncoder(nfeat, nhid, dropout, nheads)
        self.fc = nn.Linear(hidden_dim + nfeat, 1)  # Adjusted for correct dimensions

    def forward(self, smiles):
        fp_list = calculate_MACCS_fingerprints(smiles)
        fpn_out = self.fpn(fp_list)

        mols = [mol_to_graph(smile, 0) for smile in smiles]
        batched_x = torch.cat([mol.x for mol in mols], dim=0).to(device)
        
        # Correctly offset edge indices for batching
        offset = 0
        batched_edge_index = []
        for mol in mols:
            edge_index = mol.edge_index + offset  # Offset edge indices
        ...
        # Aggregate node features for each molecule
        gat_out = torch.stack([torch.mean(gat_out[offset:offset + mol.num_nodes], dim=0) for offset, mol in enumerate(mols)], dim=0)

        # Combine FPN and GAT outputs
        combined_out = torch.cat([fpn_out, gat_out], dim=1)
...
for file_name in csv_files:
    ...
    # Assuming SMILES is in the second column and target values are in the third column
    ...
    model = FpgnnModel(fp_dim=167, hidden_dim=128, dropout=0.5, nfeat=6, nhid=64, nheads=8).to(device)
    ...
    best_epoch = 0
    best_model_path = f"30-GAT-MACCS-{file_name.split('.')[0]}-model.pth"

    # Training loop
    for epoch in range(2000):
        model.train()
        optimizer.zero_grad()
        output = model(train_smiles)
        train_targets_tensor = torch.tensor(train_targets, dtype=torch.float).view(-1, 1).to(device)
        loss = criterion(output, train_targets_tensor)
        loss.backward()
        optimizer.step()
    ...
    results_name = f"30-GAT-MACCS-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'29-GAT-MACCS Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"30-GAT-MACCS-{file_name.split('.')[0]}.png"
