...
class FpgnnModel(nn.Module):
    def __init__(self, fp_dim_rdk, fp_dim_morgan, hidden_dim, dropout, nfeat, nhid, nheads):
    ...
    def forward(self, smiles):
        ...
        combined_out = torch.cat([fpn_out_rdk, fpn_out_morgan, gat_out], dim=1)
...
for file_name in csv_files:
    ...
    model = FpgnnModel(
        fp_dim_rdk=2048, fp_dim_morgan=2048, 
    ...
    best_epoch = 0
    best_model_path = f"33-GAT-RDK_Morgan-{file_name.split('.')[0]}-model.pth"
    ...
    results_name = f"33-GAT-RDK_Morgan-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'33-GAT-RDK_Morgan Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"33-GAT-RDK_Morgan-{file_name.split('.')[0]}.png"
