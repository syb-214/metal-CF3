...
# Define the FpgnnModel with combined RDK and MACCS fingerprints
class FpgnnModel(nn.Module):
    def __init__(self, fp_dim_rdk, fp_dim_maccs, hidden_dim, dropout, nfeat, nhid, nheads):
        ...
        self.fc = nn.Linear(hidden_dim * 2 + nfeat, 1)  # Adjusted for combined RDK and MACCS fingerprints
    ...
    def forward(self, smiles):
        ...
        combined_out = torch.cat([fpn_out_rdk, fpn_out_maccs, gat_out], dim=1)
...
for file_name in csv_files:
    ...
    model = FpgnnModel(
        fp_dim_rdk=2048, fp_dim_maccs=167, 
    ...
    best_epoch = 0
    best_model_path = f"32-GAT-RDK_MACCS-{file_name.split('.')[0]}-model.pth"
    ...
    results_name = f"32-GAT-RDK_MACCS-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'32-GAT-RDK_MACCS Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"32-GAT-RDK_MACCS-{file_name.split('.')[0]}.png"
