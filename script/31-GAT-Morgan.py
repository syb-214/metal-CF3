...
class FpgnnModel(nn.Module):
    ...
    def forward(self, smiles):
        fp_list = calculate_morgan_fingerprints(smiles)  # 使用 Morgan 指纹
...
for file_name in csv_files:
    ...
    model = FpgnnModel(fp_dim=2048, hidden_dim=128, dropout=0.5, nfeat=6, nhid=64, nheads=8).to(device)
    ...
    best_epoch = 0
    best_model_path = f"31-GAT-Morgan-{file_name.split('.')[0]}-model.pth"
    ...
    results_name = f"31-GAT-Morgan-{file_name.split('.')[0]}-results.txt"
    results = {
    ...
    plt.title(f'29-GAT-Morgan Prediction for {file_name}', fontsize=16)
    ...
    # Save the image
    image_name = f"31-GAT-Morgan-{file_name.split('.')[0]}.png"
