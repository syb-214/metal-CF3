...
for file_name in csv_files:
    ...
    df_MACCS = pd.DataFrame(MACCS_fingerprints, index=smiles.index)

    # Split into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(df_MACCS, targets, test_size=0.1, random_state=42)
    ...
    model_filename = "23-AdaBoost-MACCS-" + file_name.split('.')[0] + "-model.joblib"
    dump(best_model, model_filename)

    # Save the best parameters
    best_params_name = "23-AdaBoost-MACCS-" + file_name.split('.')[0] + "-best_params.txt"
    ...
    results_name = "23-AdaBoost-MACCS-" + file_name.split('.')[0] + "-results.txt"
    results = {
    ...
    cv_scores = cross_val_score(best_model, df_MACCS, targets, cv=KFold(10, shuffle=True, random_state=42), scoring='r2')
    cv_scores_name = "23-AdaBoost-MACCS-" + file_name.split('.')[0] + "-cv_scores.txt"
    ...
    plt.title('23-AdaBoost-MACCS Prediction', fontsize=16)
    ...
    # Save the image
    image_name = "23-AdaBoost-MACCS-" + file_name.split('.')[0] + ".png"
